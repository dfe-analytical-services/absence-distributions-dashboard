...
#df_absence2[, grepl("^pct", colnames(df_absence))] <- lapply(df_absence[, grepl("^pct", colnames(df_absence))], function(x) sample(0:2000, length(x), replace = TRUE))

#write.csv(df_absence, file = "data/absence_bands_distributions_dummy.csv", row.names = FALSE)
